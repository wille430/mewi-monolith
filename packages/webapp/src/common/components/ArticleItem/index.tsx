...
const ArticleItem = ({ props, id, onClick }: Props) => {
    ...
    return (
        <div className="rounded-md border-2 border-gray-300 text-xs flex flex-col shadow cursor-pointer bg-white overflow-hidden" style={{ width: '270px', height: '270px' }} onClick={e => onClick()}>
            <div className="bg-white h-2/3 border-b-2">
                <img className={`mx-auto h-full w-full ${currentImage ? 'object-cover' : 'object-cover'}`} src={currentImage || missingImage} alt={props.title} />
            </div>
            <div className="p-2 h-1/3 flex flex-col justify-between">
                ...
                <div className="w-full flex justify-between">
                    {
                        (props.price && props.price.value && props.price.currency) && <span>{props.price.value} {props.price.currency}</span>
                    }
                    {dateComponent}
                </div>
            </div>
