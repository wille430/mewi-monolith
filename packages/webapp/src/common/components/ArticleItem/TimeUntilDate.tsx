...
const TimeUntilDate = ({ endDate }: Props) => {
    ...
    const dateText = formatDistance(DateUtils.toDateObj(endDate), Date.now(), { addSuffix: true, locale: sv }).replace('ungefär', 'ca.')
    ...
    if (millisecondsAway > 0) {
        ...
        return (
            <span className="italic text-gray-400">Utgånget</span>
        )
    }
}
