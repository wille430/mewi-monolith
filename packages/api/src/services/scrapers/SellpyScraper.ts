...
export default class SellpyScraper extends Scraper {
    ...
    async getNextArticles(): Promise<ItemData[]> {
        try {
            ...
            let items: ItemData[] = data.map((item: SellpyItemData): ItemData => ({
                id: item.objectID,
                ...
                isAuction: false,
                redirectUrl: item.metadata.brand ? `https://sellpy.com/item/${item.objectID}` : null,
                price: item.pricing ? ({
