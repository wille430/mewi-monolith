...
export class SellpyEndPoint extends AbstractEndPoint<
    Listing,
    AxiosResponse,
    SellpyListing
> {
    ...
    protected extractEntities(
        res: AxiosResponse
    ): SellpyListing[] | Promise<SellpyListing[]> {
        return res.data.results[0].hits;
    }

    protected parseRawEntity(
        obj: any
    ): Partial<Listing> | Promise<Partial<Listing>> {
