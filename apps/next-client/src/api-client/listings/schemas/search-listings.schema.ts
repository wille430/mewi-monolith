...
export const searchListingsSchema = object().shape<
  Record<keyof FindAllListingsDto, any>
>({
  auction: boolean(),
  categories: array(
    mixed().oneOf(Object.values(Category) as Category[])
  ).ensure(),
  dateGte: date(),
  keyword: string(),
  limit: number().min(0),
  origins: array(
    mixed().oneOf(Object.values(ListingOrigin) as ListingOrigin[])
  ).ensure(),
  page: number().min(1),
