...
export const SideFilters = () => {
    ...
    const {resetForm} = useFormikContext<ListingSearchFilters>();
    const {isLoggedIn} = useAppSelector((state) => state.user);
    const router = useRouter();
    const [addWatcherStatus, setAddWatcherStatus] = useState<boolean | undefined>(
        undefined
    );
    ...
    return (
        <Container className="flex space-y-4">
            ...
            <div className="flex flex-col justify-end space-y-2">
                <Button onClick={() => resetForm()}>Rensa</Button>
                <ConfirmModal
                    modalProps={{
